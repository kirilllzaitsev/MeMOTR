...
from memotr.utils.utils import is_main_process
...
class Logger:
    ...
    def __init__(self, logdir: str, only_main: bool = True):
        ...
        if (self.only_main and is_main_process()) or (self.only_main is False):
            ...
            self.tb_iters_logger: tb.SummaryWriter = tb.SummaryWriter(
                log_dir=os.path.join(self.logdir, "tb_iters_log")
            )
            self.tb_epochs_logger: tb.SummaryWriter = tb.SummaryWriter(
                log_dir=os.path.join(self.logdir, "tb_epochs_log")
            )
        else:
            self.logdir = None
        ...
        if (self.only_main and is_main_process()) or (self.only_main is False):
            if isinstance(log, dict):
                ...
                if len(filename) > 5 and filename[-5:] == ".yaml":
                    ...
                    raise RuntimeError(
                        "Filename '%s' is not supported for dict log." % filename
                    )
        ...
        if (self.only_main and is_main_process()) or (self.only_main is False):
            ...
            if "total_loss" in log_keys:
                writer.add_scalar(
                    tag="loss",
                    scalar_value=(
                        log.metrics["total_loss"].avg
                        if mode == "iters"
                        else log.metrics["total_loss"].global_avg
                    ),
                    global_step=steps,
                )
        else:
