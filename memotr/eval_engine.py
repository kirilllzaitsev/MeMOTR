...
def evaluate(config: dict):
    ...
    # Tensorboard Setting
    tb_writer = tb.SummaryWriter(log_dir=os.path.join(outputs_dir, "tb"))
...
def eval_model(
    ...
    if threads > 1:
        os.system(
            f"python -m torch.distributed.run --nproc_per_node={str(threads)} --master_port={port} "
        ...
        os.system(
            f"python main.py --mode submit --submit-dir {eval_dir} --submit-model {model} "
    ...
    tracker_mv_dir = os.path.join(
        eval_dir, data_split, model.split(".")[0] + "_tracker"
    )
    ...
    if dataset_name == "DanceTrack" or dataset_name == "SportsMOT":
        ...
        raise NotImplementedError(
            f"Eval Engine DO NOT support dataset '{dataset_name}'"
        )
    if dataset_name == "DanceTrack" or dataset_name == "SportsMOT":
        os.system(
            f"python3 TrackEval/scripts/run_mot_challenge.py --SPLIT_TO_EVAL {data_split}  "
