...
import memotr.data.transforms as T
...
class MOT17(MOTDataset):
    def __init__(self, config: dict, split: str, transform):
        ...
        self.mot17_seqs_dir = os.path.join(config["DATA_ROOT"], "images", split)
        self.mot17_gts_dir = os.path.join(config["DATA_ROOT"], "gts", split)
        self.crowdhuman_seq_dir = os.path.join(
            config["DATA_ROOT"], "CrowdHuman", "images", "val"
        )
        self.crowdhuman_gts_dir = os.path.join(
            config["DATA_ROOT"], "CrowdHuman", "gts", "val"
        )
        ...
        self.mot17_seq_names = [
            seq for seq in os.listdir(self.mot17_seqs_dir) if "SDP" in seq
        ]
        for vid in self.mot17_seq_names:
            mot17_gts_dir = os.path.join(self.mot17_gts_dir, vid, "img1")
            mot17_gt_paths = [
                os.path.join(mot17_gts_dir, filename)
                for filename in os.listdir(mot17_gts_dir)
            ]
        ...
        if self.use_motsynth:
            self.motsynth_seq_names = [
                seq for seq in os.listdir(self.motsynth_seqs_dir)
            ]
            for vid in self.motsynth_seq_names:
                motsynth_gt_path = os.path.join(
                    self.motsynth_gts_dir, vid, "gt", "gt.txt"
                )
    ...
    def __len__(self):
        assert (
            self.sample_begin_frame_paths is not None
    ...
    def __getitem__(self, item):
        ...
        return {"imgs": imgs, "infos": infos, "frame_paths": frame_paths}
        ...
        if self.use_crowdhuman:
            for image_name in self.crowdhuman_gts:
                self.sample_begin_frame_paths.append(
                    os.path.join(self.crowdhuman_seq_dir, f"{image_name}.jpg")
                )
        if epoch >= self.sample_mot17_join:
            for vid in self.mot17_gts.keys():
                ...
                for t in range(t_min, t_max - (self.sample_length - 1) + 1):
                    self.sample_begin_frame_paths.append(
                        os.path.join(
                            self.mot17_seqs_dir, vid, "img1", str(t).zfill(6) + ".jpg"
                        )
                    )
        ...
        if self.use_motsynth:
            ...
            for vid in self.motsynth_gts.keys():
                ...
                for t in range(t_min, t_max - (self.sample_length - 1) + 1):
                    ...
                    self.sample_begin_frame_paths.append(
                        os.path.join(
                            self.motsynth_seqs_dir, vid, "rgb", str(t).zfill(4) + ".jpg"
                        )
                    )
            self.unified_random_state = random.getstate()
            random.setstate(multi_random_state)
    ...
    def sample_frame_paths(self, begin_frame_path: str) -> list[str]:
        ...
        if self.sample_mode == "random_interval":
            ...
            raise NotImplementedError(
                f"Do not support sample mode '{self.sample_mode}'."
            )
    ...
    def get_single_frame(self, frame_path: str):
        ...
        info["dataset"] = (
            "MOT17"
            if ("MOT17" in frame_path or "MOTSynth" in frame_path)
            else "CrowdHuman"
        )
        ...
        for i, x, y, w, h in gt:
            ...
            info["ids"].append(
                i if "MOT17" in frame_path else i + crowdhuman_ids_offset
            )
...
def transforms_for_train(
    coco_size: bool = False, overflow_bbox: bool = False, reverse_clip: bool = False
):
    ...
    return {
        "MOT17": T.MultiCompose(
            [
                ...
                #         [
                #             T.MultiRandomResize(
                #                 [400, 500, 600] if coco_size else [800, 1000, 1200]
                #             ),
                #             T.MultiRandomCrop(
...
def build(config: dict, split: str):
    if split == "train":
        ...
        raise NotImplementedError(
            f"MOT Dataset 'build' function do not support split {split}."
        )
