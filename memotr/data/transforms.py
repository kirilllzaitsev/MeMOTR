...
from memotr.utils.box_ops import box_xyxy_to_cxcywh
    ...
    def __call__(self, imgs: list, infos: list):
        if random.random() < self.p:
            ...
            def hflip(img: PIL.Image, info: dict):
                ...
                #     x2y2    x2y2            x1y2
                if len(info["boxes"]) > 0:
                    info["boxes"] = info["boxes"][:, [2, 1, 0, 3]] * torch.as_tensor(
                        [-1, 1, -1, 1]
    ...
    def __call__(self, imgs, infos):
        ...
        def resize(img, info, size, max_size):
            ...
            if isinstance(size, (list, tuple)):
                ...
                new_hw = get_new_hw(
                    current_wh=img.size, new_short_size=size, new_long_max_size=max_size
                )
            resized_img = F.resize(img, new_hw)
            ratio_w, ratio_h = (
                float(s) / float(origin_s)
                for s, origin_s in zip(resized_img.size, img.size)
            )

            assert "boxes" in info, "Info do not have key: boxes."
            assert "areas" in info, "Info do not have key: areas."
            if len(info["boxes"]) > 0:
                info["boxes"] = info["boxes"] * torch.as_tensor(
        ...
        return zip(
            *[
                resize(img, info, size=new_size, max_size=self.max_size)
...
class MultiNormalize:
    ...
    def __call__(self, imgs, infos):
        def normalize(img: torch.Tensor, info):
            assert isinstance(
                img, torch.Tensor
            ), "Image should be Tensor type before normalize."
...
class MultiRandomCrop:
    ...
    def __call__(self, imgs, infos):
        assert isinstance(
            imgs[0], PIL.Image.Image
        ), f"imgs should be PIL.Image.Image type but get {type(imgs[0])}."
        ...
        def crop(img, info, ijhw):
            ...
            if len(info["boxes"]) > 0:
                ...
                if self.overflow_bbox:  # box overflow is legal
                    ...
                    keep_idxs = torch.all(
                        torch.as_tensor(boxes[:, 1, :] > boxes[:, 0, :]), dim=1
                    )
    ...
    def __call__(self, imgs: list, infos: list):
        ...
        def shift(image, info, region, target_h, target_w):
            ...
            if "boxes" in info:
                ...
                info["boxes"] = info["boxes"].clamp(min=0)
                keep_idxs = torch.all(
                    torch.as_tensor(info["boxes"][:, 1, :] > info["boxes"][:, 0, :]),
                    dim=1,
                )
                info["boxes"] = info["boxes"].reshape(-1, 4)
