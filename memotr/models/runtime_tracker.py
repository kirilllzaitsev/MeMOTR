    ...
    def update(self, model_outputs: dict, tracks: List[TrackInstances], batch=None):
        ...
        if batch is not None and len(new_tracks) > 0 and self.matcher is not None:
            ...
            for b in range(batch_size):
                gt_trackinstances[b].ids = targets[b]["track_ids"]
                gt_trackinstances[b].labels = targets[b]["labels"]
                gt_trackinstances[b].boxes = targets[b]["boxes"]
                gt_trackinstances[b].rots = targets[b]["rot"]
                gt_trackinstances[b].other_attrs["mesh_pts"] = targets[b][
                    "mesh_pts"
                ]
                gt_trackinstances[b].other_attrs["mesh_bbox"] = targets[b][
                    "mesh_bbox"
                ]
                gt_trackinstances[b].ts = targets[b]["t"]
