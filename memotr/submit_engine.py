...
class Submitter:
    def __init__(
        ...
        result_score_thresh: float = 0.7,
        miss_tolerance: int = 5,
        use_motion: bool = False,
        motion_lambda: float = 0.5,
        ...
        self.dataloader = DataLoader(
            self.dataset, batch_size=1, num_workers=4, shuffle=False
        )
    ...
    def run(self):
        ...
        bdd100k_results = (
            []
        )  # for bdd100k, will be converted into json file, different from other datasets.
        for i, ((image, ori_image), info) in enumerate(
            tqdm(self.dataloader, desc=f"Submit seq: {self.seq_name}")
        ):
            ...
            tracks: List[TrackInstances] = get_model(
                self.model
            ).postprocess_single_frame(previous_tracks, new_tracks, None)
            ...
            if self.use_motion:
                for _ in range(len(tracks[0])):
                    if tracks[0].disappear_time[_].item() > 0:
                        if (
                            len(self.tracker.motions[tracks[0].ids[_].item()])
                            >= self.tracker.motions[
                                tracks[0].ids[_].item()
                            ].min_record_length
                        ):
                            tracks[0].ref_pts[_] = inverse_sigmoid(
                                tracks[0].last_appear_boxes[_]
                            ) + self.motion_lambda * self.tracker.motions[
                                tracks[0].ids[_].item()
                            ].get_box_delta(
                                miss_length=tracks[0].disappear_time[_].item()
                            ).to(
                                tracks[0].last_appear_boxes.device
                            )
            ...
            tracks_result = self.filter_by_score(
                tracks_result, thresh=self.result_score_thresh
            )
            ...
            if self.visualize:
                ...
                os.system(
                    f"mv ./outputs/visualize_tmp/query_updater/ ./outputs/visualize_tmp/frame_{i+1}/"
                )
                os.system(
                    f"mv ./outputs/visualize_tmp/decoder/ ./outputs/visualize_tmp/frame_{i+1}/"
                )
                os.system(
                    f"mv ./outputs/visualize_tmp/memotr/ ./outputs/visualize_tmp/frame_{i+1}/"
                )
                os.system(
                    f"mv ./outputs/visualize_tmp/runtime_tracker/ ./outputs/visualize_tmp/frame_{i+1}/"
                )
            if i >= 20:
                break
        ...
        if self.dataset_name == "BDD100K":
            with open(
                os.path.join(self.predict_dir, "{}.json".format(self.seq_name)),
                "w",
                encoding="utf-8",
            ) as f:
    ...
    def filter_by_area(tracks: TrackInstances, thresh: int = 100):
        assert len(tracks.area) == len(
            tracks.ids
        ), f"Tracks' 'area' should have the same dim with 'ids'"
        keep = tracks.area > thresh
        return tracks[keep]

    def update_results(
        self,
        tracks_result: TrackInstances,
        frame_idx: int,
        results: list,
        img_path: str,
    ):
        ...
        with open(os.path.join(self.predict_dir, f"{self.seq_name}.txt"), "a") as file:
            for i in range(len(tracks_result)):
                if (
                    ...
                    or self.dataset_name == "SportsMOT"
                    or self.dataset_name == "MOT17"
                    or self.dataset_name == "MOT17_SPLIT"
                ):
                    x1, y1, x2, y2 = tracks_result.boxes[i].tolist()
                    w, h = x2 - x1, y2 - y1
                    result_line = (
                    ...
                    raise ValueError(
                        f"{self.dataset_name} dataset is not supported for submit process."
                    )
...
def filter_by_score(tracks: TrackInstances, thresh: float = 0.7):
    return Submitter.filter_by_score(tracks=tracks, thresh=thresh)


def filter_by_area(tracks: TrackInstances, thresh: int = 100):
    return Submitter.filter_by_area(tracks=tracks, thresh=thresh)

def submit(config: dict):
    submit_logger = Logger(
        logdir=os.path.join(config["SUBMIT_DIR"], config["SUBMIT_DATA_SPLIT"]),
        only_main=True,
    )
    ...
    assert (
        config["SUBMIT_DIR"] is not None
    ), f"'--submit-dir' must not be None for submit process."
    assert (
        config["SUBMIT_MODEL"] is not None
    ), f"'--submit-model' must not be None for submit process."
    assert (
        config["SUBMIT_DATA_SPLIT"] is not None
    ), f"'--submit-data-split' must not be None for submit process."
    train_config = yaml_to_dict(
        path=path.join(config["SUBMIT_DIR"], "train/config.yaml")
    )
    ...
    load_checkpoint(
        model=model, path=path.join(config["SUBMIT_DIR"], config["SUBMIT_MODEL"])
    )
    if dataset_name == "DanceTrack" or dataset_name == "SportsMOT":
        data_split_dir = path.join(data_root, dataset_split)
    elif dataset_name == "BDD100K":
        data_split_dir = path.join(data_root, "images/track/", dataset_split)
    else:
        data_split_dir = path.join(data_root, "images", dataset_split)
    seq_names = os.listdir(data_split_dir)
    seq_names = os.listdir(data_split_dir)

    if is_distributed():
        model = DDP(
            module=model, device_ids=[distributed_rank()], find_unused_parameters=False
        )
