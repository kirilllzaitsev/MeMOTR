...
from memotr.models.utils import (
    get_model,
    load_checkpoint,
...
def train(config: dict):
    train_logger = Logger(
        logdir=os.path.join(config["OUTPUTS_DIR"], "train"), only_main=True
    )
    ...
    if config["PRETRAINED_MODEL"] is not None:
        model = load_pretrained_model(
            model, config["PRETRAINED_MODEL"], show_details=False
        )
    ...
    # Training states
    train_states = {"start_epoch": 0, "global_iters": 0}
    ...
    for epoch in range(start_epoch, config["EPOCHS"]):
        ...
        lrs = [
            optimizer.param_groups[_]["lr"] for _ in range(len(optimizer.param_groups))
        ]
        ...
        for _ in range(len(lr_names)):
            if lr_names[_] == "lr":
                default_lr_idx = _
        train_logger.tb_add_scalar(
            tag="lr", scalar_value=lrs[default_lr_idx], global_step=epoch, mode="epochs"
        )
...
def train_one_epoch(
    ...
    for i, batch in enumerate(dataloader):
        ...
        for frame_idx in range(len(batch["imgs"][0])):
            if no_grad_frames is None or frame_idx >= no_grad_frames:
                ...
                previous_tracks, new_tracks, unmatched_dets = (
                    criterion.process_single_frame(
        ...
        metric_log.update(
            name="time per iter", value=iter_end_timestamp - iter_start_timestamp
        )
        # Outputs logs
        if i % 100 == 0:
            metric_log.sync()
            ...
            logger.write(
                head=f"[Epoch={epoch}, Iter={i}/{dataloader_len}]",
                log=metric_log,
                filename="log.txt",
                mode="a",
            )
            logger.tb_add_metric_log(
                log=metric_log, steps=train_states["global_iters"], mode="iters"
            )
        ...
        if multi_checkpoint:
            if i % 100 == 0 and is_main_process():
                save_checkpoint(
                    model=model,
                    path=os.path.join(
                        logger.logdir[:-5], f"checkpoint_{int(i // 100)}.pth"
                    ),
                )
    ...
    logger.show(
        head=f"[Epoch: {epoch}, Total Time: {epoch_minutes}min]", log=metric_log
    )
    logger.write(
...
def get_param_groups(config: dict, model: nn.Module) -> Tuple[List[Dict], List[str]]:
    ...
    points_keywords = [
        "reference_points",
        "sampling_offsets",
    ]  # 在 transformer 中用于选取参考点和采样点的网络参数关键字
    ...
    param_groups = [
        {  # backbone 学习率设置
            "params": [
                ...
                if match_keywords(n, backbone_keywords) and p.requires_grad
            ],
        ...
        },
        {
            "params": [
                p
                for n, p in model.named_parameters()
                if match_keywords(n, points_keywords) and p.requires_grad
            ],
            "lr": config["LR_POINTS"],
        },
        {
            "params": [
                p
                for n, p in model.named_parameters()
                if match_keywords(n, query_updater_keywords) and p.requires_grad
            ],
            "lr": config["LR"],
        },
        {
            "params": [
                p
                for n, p in model.named_parameters()
                if not match_keywords(n, backbone_keywords)
